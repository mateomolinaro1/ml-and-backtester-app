...
class DataHandler:
    ...
    def __init__(
        ...
        self.fields_wrds_to_keep_for_universe = [
            "ticker",
            "exchcd",
            "cusip",
            "ncusip",
        ...
        self.crps_exchcd_to_currency = {1: "USD", 2: "USD", 3: "USD"}
    ...
    def connect_wrds(self):
        ...
        self.wrds_db = wrds.Connection(wrds_username=self.wrds_username, wrds_password=self.wrds_password)
    ...
    def connect_ib(self):
        ...
        if not self.ib.isConnected():
            self._manual_disconnect = False
            self._shutting_down = False
            self.ib.connect(host=self.ib_host, port=self.ib_port, clientId=self.ib_client_id)
    ...
    def on_ib_disconnected(self, *args):
        ...
        try:
            ...
            logger.error("IB reconnection failed: %s", e)
    ...
    def load_data(self):
        ...
        if self.valid_permnos_per_ib_date is None or self.valid_tickers_per_ib_date is None:
            ...
            wrds_dates = pd.DataFrame({"wrds_date": pd.to_datetime(self.wrds_universe.index.unique())}).sort_values(
                "wrds_date"
            )

            # IB dates also as a column
            ib_dates = pd.DataFrame({"ib_date": pd.to_datetime(self.universe_prices_ib.index)}).sort_values("ib_date")

            # Perform nearest-past merge
            aligned_dates = pd.merge_asof(
                ib_dates, wrds_dates, left_on="ib_date", right_on="wrds_date", direction="backward"
            ).set_index("ib_date")
    ...
    def crsp_ticker_to_ib_ticker(
        self, save_mapping_locally: bool = True, save_ib_tickers_to_cloud: bool = False
    ) -> None:
        ...
        def safe_ib_match(query: str):
            ...
            for attempt in range(self.retries):
                try:
                    ...
                    logger.error(f"Error querying IB for {query}: {exc} (attempt {attempt + 1}/{self.retries})")
                    time.sleep(1.5)
        ...
        for i, crsp_ticker in enumerate(self.tickers_across_dates):
            if i % 50 == 0:
                logger.info(
                    "matching CRSP ticker %s/%s",
                    i + 1,
                    len(self.tickers_across_dates),
                )
            ...
            if not matches or matches[0].contract.symbol in (None, ""):
                logger.info("No IB match for CRSP ticker %s", crsp_ticker)
                continue
        ...
        if save_ib_tickers_to_cloud:
            ...
            self.s3.put_object(Bucket=self.bucket_name, Key="data/ib_tickers.pkl", Body=ib_tickers_bytes)

        logger.info("CRSP -> IB ticker mapping completed successfully.")

    def fetch_wrds_historical_universe(
        self,
        wrds_request: str,
        starting_date: str,
        date_cols: List[str],
        ...
        self.wrds_gross_query = self.wrds_db.raw_sql(sql=wrds_request, date_cols=date_cols)
        ...
        self.wrds_gross_query = self.wrds_gross_query.drop_duplicates(subset=["date", "permno"], keep="last")
        # Sort for checking
        self.wrds_gross_query = self.wrds_gross_query.sort_values(by=["date"], ascending=True).reset_index(drop=True)
        ...
        if "gross_query" in saving_config:
            if saving_config["gross_query"]["extension"] == "parquet":
                self.wrds_gross_query.to_parquet(saving_config["gross_query"]["path"], index=False)
            else:
        ...
        self.format_wrds_historical_universe(
            from_cloud=crsp_to_ib_mapping_tickers_from_cloud,
        ...
        if "universe" in saving_config:
            if saving_config["universe"]["extension"] == "parquet":
                self.wrds_universe.to_parquet(saving_config["universe"]["path"], index=True)
            else:
                logger.error("Unsupported file extension for universe.")
                raise ValueError("Unsupported file extension for universe. Use 'parquet'.")

        if return_bool:
            return {"wrds_gross_query": self.wrds_gross_query, "wrds_universe": self.wrds_universe}
    ...
    def format_wrds_historical_universe(
        self, from_cloud: bool = False, update_crsp_to_ib_mapping: bool = False, save_ib_tickers_to_cloud: bool = False
    ) -> None:
            ...
            if not from_cloud:
                ...
                try:
                    ...
                    if self.s3 is not None:
                        ...
                        self.connect_aws_s3()
                    # Get the file on s3
                    crsp_to_ib_mapping_tickers = self.get_file_from_s3(
                        s3=self.s3, bucket_name=self.bucket_name, s3_object_name="data/crsp_to_ib_mapping_tickers.pkl"
                    )
        ...
        self.wrds_universe["exchange"] = self.wrds_universe["exchcd"].map(self.crsp_to_ib_exchange).fillna("UNKNOWN")
        self.wrds_universe["currency"] = (
            self.wrds_universe["exchcd"].map(self.crps_exchcd_to_currency).fillna("UNKNOWN")
        )
        ...
        if update_crsp_to_ib_mapping:
            ...
            if len(new_wrds_tickers) > 0:
                logger.info("Found %s new WRDS tickers to map to IB.", {len(new_wrds_tickers)})
                ...
                self.crsp_ticker_to_ib_ticker(
                    save_mapping_locally=False, save_ib_tickers_to_cloud=save_ib_tickers_to_cloud
                )
        ...
        self.wrds_universe["ticker_ib"] = self.wrds_universe["ticker"].map(self.crsp_to_ib_mapping_tickers)
        # Sort
        self.wrds_universe = self.wrds_universe.sort_index(ascending=True)

    def get_wrds_historical_prices(self, saving_config: dict, return_bool: bool = False) -> Union[None, pd.DataFrame]:
        """
        ...
        prices = self.wrds_gross_query.pivot(values="prc", index="date", columns="permno")
        self.universe_prices_wrds = prices

        if "prices" in saving_config:
            if saving_config["prices"]["extension"] == "parquet":
                prices.to_parquet(saving_config["prices"]["path"], index=True)
            else:
    ...
    def get_wrds_returns(self, return_bool: bool = False) -> Union[None, pd.DataFrame]:
        """
    ...
    def fetch_ib_historical_prices(
        self,
        end_date: str = "",
        past_period: str = "1 M",
        frequency: str = "1 day",
        ...
        if not load_from_cloud:
            ...
            if self.wrds_universe is None:
                ...
                if os.path.exists(file_path):
                    logger.info("Loading WRDS universe from %s", file_path)
                    self.wrds_universe = pd.read_parquet(file_path, index_col="date", parse_dates=["date"])
                else:
            ...
            if self.tickers_across_dates is None:
                ...
                if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
                    logger.info("Loading tickers from %s", file_path)
                    try:
        ...
        for i, ticker in enumerate(tickers_to_loop):
            logger.info("Fetching ticker: %s (%s/%s)", ticker, i + 1, len(tickers_to_loop))

            # Extract the most recent data
            subset = wrds_df[wrds_df["ticker"] == ticker]
            if subset.empty:
                logger.warning("! WARNING: Ticker %s not found in WRDS universe. Skipping.", ticker)
                continue
            ...
            if ib_ticker is np.nan:
                ...
                continue
            contract = Stock(
                symbol=ib_ticker, exchange=exchange_ib, primaryExchange=primary_exchange, currency=currency
            )
            ...
            ib_prices_dct[ib_ticker] = (
                df.loc[:, ["date", "close"]].set_index("date").rename(columns={"close": ib_ticker})
            )
        ...
        if save_prices:
            ...
            self.universe_prices_ib.to_parquet(self.data_path / "ib_historical_prices.parquet", index=True)
    ...
    def fetch_portfolio_value(self, saving_path_pv: str | None, return_bool: int = False) -> pd.DataFrame | None:
        if self.ib is None:
            self.connect_ib()
        values = self.ib.accountValues()
        portfolio_value = float(next(v.value for v in values if v.tag == "NetLiquidation"))
        df = pd.DataFrame(
            index=pd.Series(pd.Timestamp.today().normalize()), data=portfolio_value, columns=["portfolio_value"]
        )
    ...
    def trim_data_survivorship_free_ib(self) -> None:
        ...
        tickers_by_date = self.wrds_universe.groupby(level=0)["ticker_ib"].apply(set)
        # For each IB date, get the set of tickers valid at the corresponding WRDS date
        valid_tickers_per_ib_date = aligned_dates["wrds_date"].apply(lambda d: tickers_by_date.get(d, set()))
        permnos_by_date = self.wrds_universe.groupby(level=0)["permno"].apply(set)
        valid_permnos_per_ib_date = aligned_dates["wrds_date"].apply(lambda d: permnos_by_date.get(d, set()))
        self.valid_permnos_per_ib_date = valid_permnos_per_ib_date
        self.valid_tickers_per_ib_date = valid_tickers_per_ib_date
        mask = pd.DataFrame(
            {
                col: [col in valid_tickers_per_ib_date[date] for date in self.universe_prices_ib.index]
    ...
    def compute_coverage(self) -> None:
        ...
        coverage_crsp_tickers_to_ib_tickers = self.valid_tickers_per_ib_date.apply(
            len
        ) / self.valid_permnos_per_ib_date.apply(len)
    ...
    def get_credentials(self, return_bool: bool = False) -> Dict[str, str]:
        """
    ...
    def upload_file_to_s3(self, file_paths_and_s3_object_names: dict, bucket_name: str) -> None:
        """
        ...
        for file_path, s3_object_name in file_paths_and_s3_object_names.items():
            logger.info("Uploading %s to S3 bucket %s as %s ...", file_path, bucket_name, s3_object_name)
            ...
            self.s3.upload_file(Filename=file_path, Bucket=bucket_name, Key=s3_object_name)
    ...
    def check_files_on_s3(self) -> None:
        ...
        for _, s3_object_name in self.file_paths_and_s3_object_names.items():
            logger.info("Checking existence of %s in S3 bucket %s ...", s3_object_name, self.bucket_name)
            try:
                self.s3.head_object(Bucket=self.bucket_name, Key=s3_object_name)
                logger.info("File %s exists in S3 bucket %s.", s3_object_name, self.bucket_name)
            except Exception as e:
    ...
    @staticmethod
    def get_file_from_s3(s3: BaseClient, bucket_name: str, s3_object_name: str) -> bytes:
        """
    ...
    def get_files_from_s3(self) -> None:
        ...
        if self.file_paths_and_s3_object_names is not None and self.s3_files_downloaded is None:
            self.s3_files_downloaded = {
                Path(file_name).stem: None for file_name in self.file_paths_and_s3_object_names.keys()
            }
            for _, s3_object_name in self.file_paths_and_s3_object_names.items():
                logger.info("Downloading %s from S3 bucket %s", s3_object_name, self.bucket_name)

                response = self.s3.get_object(Bucket=self.bucket_name, Key=s3_object_name)
                file_content = response["Body"].read()
    ...
    def update_wrds_data(
        self, wrds_request: str, date_cols: List[str], saving_config: dict, return_bool: bool = False
    ) -> dict:
        """
        ...
        self.fetch_wrds_historical_universe(
        ...
        updated_wrds_gross_query = pd.concat(
            [current_wrds_gross_query, self.wrds_gross_query], axis=0, ignore_index=True
        )
        ...
        if first_date_new_universe <= last_date_current_universe:
            ...
            return {}
        updated_wrds_universe = pd.concat([current_wrds_universe, self.wrds_universe], axis=0, ignore_index=False)
        ...
        return {
            "data/wrds_gross_query.parquet": updated_wrds_gross_query,
    ...
    def update_ib_data(self):
        ...
        if self.ib is None or not self.ib.isConnected():
            self.connect_ib()
        if (
            self.s3_files_downloaded is None
            or "ib_historical_prices" not in self.s3_files_downloaded
            or "ib_tickers" not in self.s3_files_downloaded
        ):
        ...
        new_ib_prices = self.fetch_ib_historical_prices(
            end_date="",
        ...
        if first_new_date <= last_date_in_current:
            ...
            return
        # add new dates
        updated_ib_historical_prices = pd.concat(
            [
                updated_ib_historical_prices,
                pd.DataFrame(index=new_ib_prices.index, columns=updated_ib_historical_prices.columns, data=np.nan),
            ],
            axis=0,
            ignore_index=False,
        )
        ...
        return {
            "data/ib_historical_prices.parquet": updated_ib_historical_prices,
    ...
    def update_portfolio_value(self):
        ...
        if first_date_new <= last_date_old:
            ...
            missing_dates = pd.date_range(start=last_date_old, end=first_date_new, inclusive="neither")
            if missing_dates.empty:
                # just concatenate
                updated_pv = pd.concat([old_pv, new_pv], axis=0)
                updated_pv = updated_pv.sort_index(ascending=True)
            else:
                df_missing = pd.DataFrame(index=missing_dates, data=np.nan, columns=["portfolio_value"])
    ...
    def update_data(
        ...
        updated_wrds_objects = self.update_wrds_data(
            wrds_request=wrds_request, date_cols=date_cols, saving_config=saving_config, return_bool=return_bool
        )
        # Step 3.1: upload updated wrds files to s3
        if updated_wrds_objects:
            logger.info("New WRDS data to update.")
            s3Utils.replace_existing_files_in_s3(
                s3=self.s3, bucket_name=self.bucket_name, files_dct=updated_wrds_objects
            )
        ...
        while (notna_current / notna_prevdate < coverage_threshold) and (tries <= max_tries):
            ...
            tries += 1
            logger.info(
                "We got poor coverage of non nan prices while updating. Redoing the update for the %s time.", tries
            )

        # Step 4.1: upload updated ib files to s3
        s3Utils.replace_existing_files_in_s3(s3=self.s3, bucket_name=self.bucket_name, files_dct=updated_ib_objects)
